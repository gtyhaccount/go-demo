...
func main() {
	...
	*/
	var a string
	a = "   a a   "
	fmt.Println("判断是否以某个字符开头", strings.HasPrefix(strings.TrimSpace(a), "a"))

	fmt.Println("判断字符串是否相等", strings.Contains(strings.TrimSpace(strings.ToLower(" O PPO ")), "oppo"))

	g := Goo("as")
	fmt.Println(g)

	strings.Contains(strings.TrimSpace(strings.ToLower("")), "oppo")
